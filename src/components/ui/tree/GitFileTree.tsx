    ...
    return (
        <div className="flex flex-col">
            {sortedEntries(tree.children).map(([name, node]) => (
                <GitTreeNode key={name} name={name} node={node} depth={0} selectedFile={selectedFile} />
            ))}
        </div>
    );
}
...
function GitTreeNode({ name, node, depth, selectedFile }: { name: string; node: FileTreeNode; depth: number; selectedFile?: string }) {
    ...
    return (
        <div className="w-full select-none">
            {node.filePath ? (
                <Link to="/$lang/studio/editor/github/diff" params={{ lang }} search={{ file: node.filePath }}>
                    {content}
                </Link>
            ) : (
            ...
            {hasChildren && isOpen && (
                <div className="flex flex-col border-zinc-800/50 my-0.5 pl-1 ml-3 border-l">
                    {sortedEntries(node.children).map(([childName, childNode]) => (
                        <GitTreeNode key={childName} name={childName} node={childNode} depth={depth + 1} selectedFile={selectedFile} />
                    ))}
                </div>
            )}
        </div>
