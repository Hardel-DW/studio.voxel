import { createContext, useContext, useState, useEffect, cloneElement, isValidElement, type ReactNode, type MouseEvent, type ComponentPropsWithoutRef } from "react"
...
export function DropZone({ children, zone, onDrop, asChild = false, ...props }: DropZoneProps) {
    ...
    const dropProps = {
        ...
        onMouseEnter: () => context.dragData && setIsHovered(true),
        onMouseLeave: () => setIsHovered(false),
        className: cn(
            "transition-colors",
            props?.className,
            isHovered && context.dragData && "border-zinc-500 border-dashed bg-black/50",
        )
    }
