...
export default function MigrationUpload({ id, onFileUpload, title, description }: MigrationUploadProps) {
    return (
        <div className="w-full h-full min-h-[250px] flex flex-col">
            <Dropzone id={id} dropzone={{ accept: ".zip", maxSize: 100000000, multiple: false }} onFileUpload={onFileUpload}>
                <div className="flex flex-col items-center justify-center gap-6 p-8 transition-all duration-300 group-hover:bg-zinc-800/30 rounded-3xl border-2 border-dashed border-zinc-700/50 group-hover:border-zinc-500 h-full min-h-[250px] cursor-pointer bg-zinc-900/20 backdrop-blur-sm">
