...
export function ToolNavItem({ title, description, image, href, alignRight, comingSoon, elementsCount, index }: ToolNavItemProps) {
    ...
    return (
        <button
            ...
            disabled={comingSoon}
        >
            <div className="absolute inset-0 -z-10 brightness-20 hue-rotate-90 blur-lg" style={{ transform: `translateX(${-200 + index * 100}px)` }}>
            ...
            <div className={cn("flex items-center gap-4", alignRight && "flex-row-reverse")}>
                <div className={cn(
                    ...
                    <img src={image} alt="Tool" className={cn(
                        "w-full h-full pixelated",
            ...
            {!comingSoon && (
                <div className={cn(
                    ...
                    <svg className="w-12 h-12 text-zinc-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={alignRight ? "M7 17l-4-4m0 0l4-4m-4 4h18" : "M17 8l4 4m0 0l-4 4m4-4H3"} />
                    </svg>
                </div>
            )}
        </button>
