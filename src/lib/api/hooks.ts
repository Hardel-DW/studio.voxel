...
export const useDownloadRepo = () => useMutation({ mutationFn: ({ owner, repo, branch }: { owner: string; repo: string; branch: string }) => downloadRepo(owner, repo, branch) });

export const useInitializeRepository = () => useMutation({
    mutationFn: ({ name, description, isPrivate, autoInit, files }: {
        name: string;
        description: string;
        isPrivate: boolean;
        autoInit: boolean;
...
export const usePushToGitHub = () => useMutation({
    mutationFn: ({ owner, repo, branch, files }: {
        owner: string;
        repo: string;
        branch: string;
        files: Record<string, string | null>;
...
export const useCreatePullRequest = () => useMutation({
    mutationFn: ({ owner, repo, branch, files, newBranch }: {
        owner: string;
        repo: string;
        branch: string;
        files: Record<string, string | null>;
